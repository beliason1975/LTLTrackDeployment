...
/****** Object:  StoredProcedure [dbo].[Select_CoyoteTrackingProStatus2013]    Script Date: 06/12/2017 5:44:07 PM ******/
SET ANSI_NULLS ON
GO
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Select_CoyoteTrackingProStatus2013]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[Select_CoyoteTrackingProStatus2013] AS'
END
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[Select_CoyoteTrackingProStatus2013] (
	@pProNumber integer,
	@webSite bit
)
AS
BEGIN
	...
	BEGIN */
		...
		DECLARE @query varchar(max) =
			...
			where ALCO = 1 AND ALPRO = ' + cast(@pProNumber as varchar) + '
		...
		SET @query = 'SElECT * FROM OpenQuery(COYOTE, ''SELECT BSPRO AS ProNumber,
			...
			WHERE BSCONO = 1 and BSPRO = ' + cast(@pProNumber as varchar) + ' '')'
...
--***********************************************************************
		...
		WHERE [BACO#] = 1
			AND BBPRO = ' + cast(@pProNumber as varchar) + '
			AND BATRPS <> 0
	...
	IF @webSite = 1
		BEGIN
			...
			FROM #status t --RoadrunnerCentral.dbo.TrackingStatusCache2013 t
			 WHERE t.ProNumber = @pProNumber AND t.StatusCode NOT IN ('DUD', 'INV')
