/****** Object:  StoredProcedure [dbo].[Select_CoyoteTrackingProHeader2013]    Script Date: 06/12/2017 5:42:38 PM ******/
SET ANSI_NULLS ON
GO
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Select_CoyoteTrackingProHeader2013]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[Select_CoyoteTrackingProHeader2013] AS'
END
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[Select_CoyoteTrackingProHeader2013] (
	@pProNumber integer,
	@webSite bit
)
AS
BEGIN
	/*IF NOT EXISTS (SELECT * FROM [SPINSQLC01\INTERNET].[RoadrunnerCentral].dbo.TrackingCache2013
	...
	LEFT JOIN [SPINSQLC01\INTERNET].[RoadrunnerCentral].[dbo].[ContactMethod] cm
		ON lcm.ContactMethodID = cm.ContactMethodID
	WHERE l.LocationCode = #track.OriginTerminalName
	AND cm.[Name] = 'Toll Free'

/*	INSERT INTO [SPINSQLC01\INTERNET].[RoadrunnerCentral].dbo.TrackingCache2013
	([ProNumber] ,[OriginName] ,[OriginAddress1] ,[OriginAddress2]
      ,[OriginCity] ,[OriginState] ,[OriginPostalCode] ,[DestinationName]
      ,[DestinationAddress1] ,[DestinationAddress2] ,[DestinationCity]
      ,[DestinationState] ,[DestinationPostalCode] ,[CustomerNumber]
      ,[BOLNumber] ,[PONumber] ,[Pieces] ,[Weight] ,[AppointmentDate]
      ,[AppointmentTime] ,[EstimatedDeliveryDate] ,[DeliveredDate]
      ,[BilltoNumber] ,[ProjectedDeliveryDate] ,[DeliveredTime] ,[HAWB]
      ,[OriginTerminalName] ,[OriginTerminalPhone] ,[BOLReceived]
      ,[PODReceived]
		) */
		SELECT [ProNumber]
      ,[OriginName]
	...
	BEGIN
		...
		SELECT *, @Manifest AS Manifest FROM #TrackProHeader --[SPINSQLC01\INTERNET].[RoadrunnerCentral].dbo.TrackingCache2013
		WHERE ProNumber = @pProNumber
	END

	DROP TABLE #TrackProHeader
