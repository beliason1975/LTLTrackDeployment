...
/****** Object:  StoredProcedure [dbo].[GetMcLeodTrackingByPro]    Script Date: 06/22/2017 5:19:06 PM ******/
SET ANSI_NULLS ON
GO
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GetMcLeodTrackingByPro]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[GetMcLeodTrackingByPro] AS'
END
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[GetMcLeodTrackingByPro]
...
BEGIN
	...
	BEGIN CATCH
		...
		IF @vErrSource IS NULL
			BEGIN
				SET @vErrSource = 'STORED PROC GetMcLeodTrackingByPro'
			END

		EXEC spErrorTraceLog_Insert @vErrDescription, @vErrSource, @vErrMessage, 'EXEC GetMcLeodTrackingByPro'
