				...
				FROM MCLEOD.lme_1510_ltl.dbo.orders o with(nolock)
					INNER JOIN MCLEOD.lme_1510_ltl.dbo.stop ship with(nolock) ON --Shipper Info
						o.shipper_stop_id = ship.id AND
						o.company_id = ship.company_id AND
						ship.stop_type = 'PU'
					LEFT JOIN MCLEOD.lme_1510_ltl.dbo.stop con with(nolock) ON  --Consignee Info
						o.consignee_stop_id = con.id AND
						o.company_id = con.company_id AND
						con.stop_type = 'SO'
					INNER JOIN MCLEOD.lme_1510_ltl.dbo.freight_group fg with(nolock) ON
					...
					LEFT JOIN (SELECT
							...
							FROM  [Location] l
								LEFT JOIN [LocationContactMethodValue] lcm
				...
				WHERE o.operational_status = 'PICK'  AND fg.pro_nbr = '399435809'--and o.ordered_date >= datediff(day, 0, getdate()) --
